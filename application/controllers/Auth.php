...
class Auth extends CI_Controller
{
	...
	function read(){
		...
		$data['results'] = $this->users->get_users();
		...
		$this->load->view('users_read_view', $data);  //note: no .php extension
	...
	function edit($id){
		...
		$this->form_validation->set_rules('admin', 'Admin Privileges', 'max_length[1]|trim|strip_tags');
		$this->form_validation->set_rules('super_admin', 'Admin Privileges', 'max_length[1]|trim|strip_tags');

		if($this->form_validation->run() == FALSE){
			$data['results'] = $this->users->get_user_detail($id);
		...
		if ($this->tank_auth->is_logged_in()) {	
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
					...
					$activated = $this->session->userdata('status');
					$data2 =$this->users->get_user_by_id($id,$activated);

					// $data2['results'] = $this->users->get_user_detail($id);
			 	if($data2){
			...
			/*	foreach($data2 as $row){
				...
				$this->session->set_userdata('admin',$data2->admin);
				$this->session->set_userdata('super_admin', $data2->super_admin);	
				}// if results
						//$this->session->set_userdata('admin','3');	
